...
For each fold in the 10-fold cross-validation, the SMOTE and Tomek techniques were all only applied training samples. This resulted in a kNN mean accuracy of 0.9089 across the 10 folds. The accuracy scores ranged from 0.9065 to 0.9116 which showed that the algorithm generalises well.

\begin{table}[htbp]
	\centering
	\caption{Performance Comparison of KNN and Decision Tree Models}
	\label{tab:model_comparison}
	\begin{tabular}{|l|c|c|c|c|}
		\hline
		\textbf{Model} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} \\
		\hline
		KNN & $0.9089 \pm 0.0028$ & $0.83$ & $0.82$ & $0.82$ \\
		\hline
		Decision Tree & $0.9160 \pm 0.0051$ & $0.92$ & $0.92$ & $0.92$ \\
		\hline
	\end{tabular}
\end{table}

\subsubsection{Classification Trees}
For the classification trees 10-fold cross validation was performed with SMOTE only applied to the training folds. The maximum accuracy of all runs was 0.9202 and the minimum was 0.9125. The mean accuracy of 0.9160 and other metrics can be observed in Table~\ref{tab:model_comparison}. The very low standard deviation of 0.0051 on the accuracy metric indicates that the model has stable performance.
