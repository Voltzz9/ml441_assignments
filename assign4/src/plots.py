...
def plot_comprehensive_analysis(param_sizes, precision_mean, precision_std, recall_mean, recall_std,
                                f1_mean, f1_std, training_times, param_name='n_estimators', 
                                save_path=None):
    """
    Creates a comprehensive 3-panel figure with all key analyses.
    ...
    ax = axes[0]
    ax.plot(param_sizes, f1_mean, marker='o', label='F1 Score', linewidth=2, markersize=6)
    ax.fill_between(param_sizes, 
                     np.array(f1_mean) - np.array(f1_std), 
                     np.array(f1_mean) + np.array(f1_std), 
                     alpha=0.2)
    ax.plot(param_sizes, precision_mean, marker='s', label='Precision', linewidth=2, markersize=6)
    ax.plot(param_sizes, recall_mean, marker='^', label='Recall', linewidth=2, markersize=6)
    ax.set_xscale('log')
    ax.set_xlabel(f'{param_name}', fontsize=20)
    ax.set_ylabel('Score', fontsize=20)
    ax.set_ylim([0.5, 1.0])
    ax.legend(loc='best', fontsize=16)
    ax.grid(True, alpha=0.3)
    ax.set_title('Convergence Curve', fontsize=22, fontweight='bold')
    
    # Panel 2: Stability Analysis
    ax = axes[1]
    cv = np.array(f1_std) / np.array(f1_mean)
    ax.plot(param_sizes, cv, marker='o', linewidth=2, markersize=6, color='crimson')
    ax.set_xscale('log')
    ax.set_xlabel(f'{param_name}', fontsize=20)
    ax.set_ylabel('Coefficient of Variation', fontsize=20)
    ax.grid(True, alpha=0.3)
    ax.set_title('Stability Analysis', fontsize=22, fontweight='bold')
    
    # Panel 3: Performance vs Time
    ax = axes[2]
    ax.errorbar(training_times, f1_mean, yerr=f1_std, 
                marker='o', linewidth=2, markersize=6, 
                capsize=4, capthick=1.5, color='green')
    ax.set_xscale('log')
    ax.set_xlabel('Training Time (s)', fontsize=20)
    ax.set_ylabel('F1 Score', fontsize=20)
    ax.grid(True, alpha=0.3)
    ax.set_title('Performance vs Time', fontsize=22, fontweight='bold')
